...
.text
	...
	addi $s1, $zero, 1 			#numCalls = 1
	addi $s2, $zero, -1			#Store -1 
...
loop:
	add $a0, $zero, $s1			#Pass in numcalls 
	jal	_playSequence			#Play the sequence 
	la $a1, sequence
	addi $s3, $zero, 0 
	Increment: 
		beq $s3, $s0, Incrementdone 
		addi $a1, $a1, 4
		addi $s3, $s3, 1 
		j	Increment 
...
eLoop:	bne $t8, $zero, eLoop
	addi $s1, $zero, 1 			#Reset variables 
	addi $s2, $zero, -1
	addi $s0, $zero, 0
