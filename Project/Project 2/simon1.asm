...
loop:
	...
	sw  $v0, sequence($s0)  		#Add the new sequence to the end
	addi $s0, $s0, 4
	sw   $t1, sequence($s0)			#Terminate with a -1 
...
_playSequence: 
	...
	addi $s1, $zero, -1			#s1 = -1 
loop1:	
	lw $a0, sequence($s3)   		#a0 = sequence[index] 
	beq $a0, $s1, done1			#if a0 = -1 jump to done1 
	jal	_makeNoise			#Make the noise 
	addiu $s3, $s3, 4			#increment index
...
_userPlay:
	addi $sp, $sp, -16			#Save to the stack 
	sw $ra, ($sp)
	sw $s0, 4($sp)
	sw $s1, 8($sp)
	sw $s2, 12($sp)
...
uLoop: 
	...
	jal	_makeNoiseUser	
	lw $s1, sequence($s2)  			#Compare the selected color to the once in the sequence 
	beq $s1, 0, blueSet 
	beq $s1, 1, yellowSet
	beq $s1, 2, greenSet
	beq $s1, 3, redSet
return:	bne $t9, $s1, notEqual			#If the selected color is not equal leave loop 
