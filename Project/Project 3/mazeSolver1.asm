...
#adds 0 to array for foward run. 
#adds 1 to array for every left turn
#adds 2 for every right turn. 
...
_cleanSolution:
	...
	addi $s6, $s3, 0 	#Temp 
	CleanLoop:  	#beq $s2, $s6, exit		#Reached the end of the array 
			lw $s0, solution($s2)
			addi $s2, $s2, 4 
			lw $s1, solution($s2)
		 	addi $s3, $s6, 0 
...
skipCol: 
	lw $s4, solution($s2)
	slt $s5, $s4, $s1		#if current postion greater than array col move east
	beq $s5, 1, moveWestT
	slt $s5, $s1, $s4		#if current postion is lesst than array col move west
	beq $s5, 1, moveEastT
	subi $s2, $s2, 4
	lw $s3, solution($s2)
	slt $s5, $s3, $s0
